...
def nucleotide_scores(data, main_model, main_params, explainer_dev, explainer_hk):
    ...
    for d in shap_values_both:
        print(d.shape)
    print(X_cand_specific_enh.shape)
    ...
    for i in [0,4,13,14]:
        ...
        for t in [0,1]: # dev and hk
            ...
            plt.savefig(f"plot_weights_t{t}_i{i}.png", dpi=300, bbox_inches='tight')
            plt.close()  

def main(config, indir, mdldir):
    ...
    explainer_hk = shap.DeepExplainer((main_model.layers[0].input, main_model.layers[-1].output),
                                    data=background)
    nucleotide_scores(data, main_model, main_params, explainer_dev, explainer_hk)

if (len(sys.argv)!=4):
    exit("<parms.config> <data-indir> <mdl-dir>\n")

(config, indir, mdldir)=sys.argv[1:]
main(config, indir, mdldir)